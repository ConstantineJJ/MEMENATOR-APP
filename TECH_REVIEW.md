...
- [ ] Split oversized `server.ts`, `MemeCanvas.tsx`, and `App.tsx` without changing behavior. Progress: `App.tsx` orchestration has been split into `useMemeUndoHistory`, `useMemeDraftPersistence`, `useMemeHistoryAutosave`, `useCompositionAnalysis`, and `useMagicCaptions`; dead template-selection code and the removed filters/stickers UI plumbing were cleaned up. Next: split `MemeCanvas` toolbar/export/interaction concerns, then modularize `server.ts` routes/services.
